...
const Hero = ({hasAnimated})=>{
    ...
    return (
        <section className="relative h-screen flex items-center justify-center bg-gradient-to-br from-white via-gray-50 to-gray-100 pt-20">
            ...
            <div className="relative z-10 max-w-6xl mx-auto px-6 text-center h-full flex flex-col justify-center">
                <div className={`transition-all duration-1000 ${hasAnimated.hero? 'opacity-100 translate-y-0':'opacity-0 translate-y-10'}`} id="hero">
                    ...
                    {/* name */}
                    <h1 className="text-3xl md:text-5xl font-bold mb-4 bg-gradient-to-r from-black via-gray-800 to-gray-600 bg-clip-text text-transparent animate-fade-in-up">
                        {heroData.name}
                    </h1>

                    <div className="h-12 md:h-16 ,b-6 flex items-center justify-center">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 justify-center mb-8 animate-fade-in-up delay-300">
                        {heroData.ctaButtons.map((button, index)=>(
                            <a key={index} href={button.href} className={`group relative px-5 py-2.5 rounded-lg transition-all duration-300 font-medium text-sm ${button.variants === 'primary'? 'bg-black text-white shadow-lg hover:shadow-xl hover:bg-gray-800' : 'border-2 border-black text-black hover:bg-black hover:text-white'}`}>
                                ...
                                <span className={button.variants === 'primary' ? 'relative z-10': ''}>
                                    {button.text}
                                </span>
                            </a>
                        ))}
                        ...
                        {heroData.socialLinks.map((social, index)=>{
                            ...
                            return (
                                <a key={index} href={social.url} className="group p-3 bg-white rounded-full shadow-md hover:shadow-lg transition-all duration-300 transform hover:scale-110 hover:-translate-y-1">
                                    <IconComponent className="w-6 h-6 text-gray-700 group-hover:text-black transition-colors duration-300"/>
                                </a>
                            )
                        })}
                    </div>
                    ...
                    <div className="animate-bounce">
                        <button onClick={scrollToSection} className="group flex flex-col items-center text-gray-600 hover:text-black transition-colors cursor-pointer" >
            ...
            <div className="absolute bottom-20 right-10 w-5 h-5 bg-orange-400 rounded-full opacity-60 animate-float delay-1000"></div>
